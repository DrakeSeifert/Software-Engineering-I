...
class Kwic():
    ...
    mystr = ""
    finalproduct = []
    myfsm = EventSpec("kwic.fsm")

    def __init__(self,periodsToBreaks=False, ignoreWords=[]):
		Kwic.myfsm.event("callConstructor")
    ...
    def addText(self,newString):
		Kwic.myfsm.event("callAddText")
		Kwic.myfsm.event("appendString")
		Kwic.mystr += newString
		Kwic.finalproduct = self.runkwic()

    def index(self):
		Kwic.myfsm.event("callIndex")
		Kwic.myfsm.event("returnIndex")
		if Kwic.finalproduct == []:
			return []
		else:
			return Kwic.finalproduct[0]

    def listPairs(self):
		Kwic.myfsm.event("callListPairs")
		Kwic.myfsm.event("returnListPairs")
		if Kwic.finalproduct == []:
			return []
		else:
			return Kwic.finalproduct[1]

    def reset(self):
		Kwic.myfsm.event("callReset")
		Kwic.myfsm.event("clearString")
		Kwic.mystr = ""
		Kwic.finalproduct = self.runkwic()

    def runkwic(self):
	Kwic.myfsm.event("callRunKwic")
	if(Kwic.mystr == ""):
		emptylist = []
		return emptylist
