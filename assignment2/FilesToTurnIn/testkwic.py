...
def strIsEmpty(mystr):
	if(mystr == ""):
		return True

#split string by line, returns list of sentences
def breakLines(mystr, **myParameter):
...
#apply ignoreWords parameter if true
def applyIgnoreWords(shift, **myParameter): 
...
#remove grammar markings and return finalproduct as a tuple
def stripGrammarMarkings(shift):
...
def applyListPairs(splitword, listPairShift, finalproduct):
...
def kwic(mystr, **myParameter):
	...
	#return emptylist for emptystring
	if(strIsEmpty(mystr)):
		emptylist = []
		return emptylist
	
	#split string by line, returns list of sentences
	splitline = breakLines(mystr, **myParameter)	
	
	#split sentences by word, making splitword a list of lists
	splitword = breakWords(splitline)

	#put each inner list into a tuple, paired with its line number
	tupledList = addLineNumber(splitword)

	#Perform shifting operation
	shift = shiftLines(splitword)

	#make a copy in case listPairShift == True
	listPairShift = deepcopy(shift)

	#check ignoreWords parameter and assign final product
	#to be returned
	if ('ignoreWords' in myParameter):
		shift = applyIgnoreWords(shift, **myParameter)

	#remove grammar markings and return finalproduct as a tuple
	finalproduct = stripGrammarMarkings(shift)

	if ('listPairs' in myParameter):
		if(myParameter['listPairs'] == True):
			return applyListPairs(splitword, \
					      listPairShift, \
					      finalproduct)
