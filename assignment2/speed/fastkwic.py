...
def kwic(mystr, **myParameter):
	...
	strippedString = mystr.replace('\n','')
	if os.path.isfile('./previousEntries.txt'):
		print "Entry previously entered, fetching data...\n"
		if strippedString in open('./previousEntries.txt').read():
			with open('./previousEntries.txt', 'r') as f:
				for line in f:
					if strippedString in line:
						for line in f:
							answer = line
							finalAnswer = ast.literal_eval(answer)
							print "SUCCESS"
							return finalAnswer
	...
	casted = str(finalproduct)

	f = open('previousEntries.txt', 'a')
	f.write('\n|newEntry|\n')
	f.write(mystr.replace('\n',''))
	f.write('\n')
	f.write(casted)
	f.close()
